...
export function WatchPage(props){
  ...
  const countView = async () => {
    await axios.put(BASE_URL+"/stats/video/countview/"+id);
  }
  const likesCount= async () => {
    await axios.get(BASE_URL+"/stats/video/likes/"+id).then(res =>{
      setLikes(res.data)
    })
  }
  const viewsCount = async () => {
    await axios.get(BASE_URL+"/stats/video/views/"+id).then(res =>{
      setViews(res.data);
    })
  }
  useEffect(() => {
